 ...
 *Description: 简易分段环形缓冲的实现
 *Others: Reference by netube_99\netube@163.com
 *History:
		1、Date:
	     Author:
	     Modification:
...
uint8_t		buffer_base[BASE_SIZE] = {0};						//用于存储数据
uint32_t	buffer_chapter[CHAPTER_SIZE] = {0};			//用于存储分段数量

ring_buffer_chapter  rbc = {0};										//创建分段环形缓冲区句柄
